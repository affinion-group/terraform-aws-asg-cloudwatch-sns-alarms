module "asg_cpu_single_high" {
  ...
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["cpu", "single", "high"]))
}
...
module "asg_cpu_high" {
  ...
  namespace  = var.namespace
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["cpu", "average", "high"]))
}
...
module "asg_maxed_out" {
  ...
  namespace  = var.namespace
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["asg", "maxed", "out"]))
}
